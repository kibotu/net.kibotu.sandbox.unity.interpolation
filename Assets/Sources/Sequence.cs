...
namespace Assets.Sources
{
    public class Sequence : MonoBehaviour
    {
        ...
        public void StartCube(int type)
        {
            ...
            {
                case (int)Type.Line:
                    Path[0].LoopMoving().OnFulfilled += result => Path[1].LoopMoving().OnFulfilled += result2 => Path[2].LoopMoving().OnFulfilled +=  vector3 => _isRunning = false;
                    break;
